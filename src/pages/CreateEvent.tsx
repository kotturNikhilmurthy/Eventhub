...
const CreateEvent = () => {
  ...
  const generateFamilyId = () => {
    const events = JSON.parse(localStorage.getItem("events") || "[]") as { type?: string }[];
    const familyEvents = events.filter((e) => e.type === "Family");
    return "FAM" + String(familyEvents.length + 1).padStart(3, "0");
  };
  ...
  const createEvent = async (withAd: boolean = false) => {
    const eventId = generateEventId();
    const familyId = formData.type === "Family" ? generateFamilyId() : undefined;
    const { familyCategory, brideName, groomName, birthdayName, birthdayYear, ...baseForm } = formData;

    let computedName = baseForm.name;
    if (formData.type === "Family") {
      if (familyCategory === "Marriage") {
        ...
        computedName = birthdayYear.trim()
          ? `${celebrant} - ${birthdayYear.trim()}`
          : celebrant;
      }
    }
    ...
    const familyDetails =
      formData.type === "Family"
        ? familyCategory === "Marriage"
          ? {
              type: "Marriage" as const,
              brideName: brideName.trim(),
              groomName: groomName.trim(),
            }
          : familyCategory === "Birthday"
            ? {
                type: "Birthday" as const,
                celebrantName: birthdayName.trim(),
                celebrationYear: birthdayYear.trim(),
              }
            : undefined
        : undefined;
    ...
    const newEvent = {
      ...baseForm,
      name: computedName,
      ticketPrice: formData.type === "Family" ? "0" : formData.ticketPrice,
      theatres: selectedTheatres,
      showtimes: selectedShowtimes,
      familyId,
      status: "Active",
      capacity: 100, // Default capacity
      ...(formData.type === "Family" && { familyCategory }),
    ...
    try {
      ...
      if (withAd && promoteEvent && adImage) {
        ...
        expiryDate.setDate(expiryDate.getDate() + parseInt(adDuration));

        const advertisement = {
          id: "AD" + Date.now(),
          eventId: createdEvent._id || createdEvent.id,
        ...
        // Still store ads in localStorage for now
        ...
        toast({
          ...
          description: `Your event is now featured on the homepage for ${adDuration} days`,
        });
      } else {
        toast({
          title: familyId ? "Family Event Created!" : "Event Created!",
          description: familyId ? `Family Event ID: ${familyId}` : "Event created successfully",
        });
      }
